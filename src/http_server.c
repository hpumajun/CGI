...
static void analysis_request(int buffer_len,char * request,char *method,char *url,char *version)
{
    int i = 0;
    int j = 0;
    while ((i < (sizeof(method) - 1)) && (!isspace(request[i]))) {
            method[i] = request[i];
            i++;
        }
        method[i] = '\0';
        ...
        // Get URL
        j = 0;
        while (i < buffer_len && (j < (sizeof(url) - 1)) && !isspace(request[i])) {
            url[j] = request[i];
            i++;
            j++;
        }
        ...
        j = 0;
        while (j < sizeof(version) - 1 && !isspace(request[i])) {
            version[j] = request[i];
            i++;
            j++;
        }
...
int handle_client_connection() {
    ...
    analysis_request(buffer_len,buffer,method,url,version);
    st_session.fd = client_sockfd;
    st_session.pszmethod = method;
    st_session.pszurl = url;
    st_session.pszversion = version;
    ...
    msg(M_INFO,"operate is **%d**",request);
...
int http_server(void) {
    ...
    while(TRUE)
    {
        ...
        if (client_sockfd < 0) {
            msg(M_INFO,"Unable to accept connection");
            terminate(1);
            return 1;
        }
        ...
        if (client_entity > 0) {
            msg(M_INFO,"[%s] Connection accepted from %s (%s)", timestamp_str, client_entity->h_name, client_entity->h_addr_list[0]);
        } else {
